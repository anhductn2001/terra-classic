...
func (s *IntegrationTestSuite) TestByPassMinFee() {
	...
	chainAPI := fmt.Sprintf("http://%s", s.valResources[s.chainA.ChainMeta.Id][0].GetHostPort("1317/tcp"))
...
func (s *IntegrationTestSuite) TestVesting() {
	...
	chainAAPI := fmt.Sprintf("http://%s", s.valResources[s.chainA.ChainMeta.Id][0].GetHostPort("1317/tcp"))
